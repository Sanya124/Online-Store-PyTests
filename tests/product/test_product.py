...
class TestProduct:
    ...
    @description('WHEN requesting to get by product ID, THEN product info is returned')
    def test_get_product(self, auth, postgres):
        ...
        with step('Getting product info by ID via API'):
            data = ProductAPI().get_by_id(token=token, _id=db_data['id'])
            assert_data = data.json()

        with step('Checking mapping data DB <> API'):
            ProductAsserts.check_mapping_db_to_api(reference=assert_data, compared=db_data)

    @title('Getting product info by ID not authorized')
    @description('WHEN not authorized requesting to get product info by ID, THEN product info is returned')
    ...
    @description('WHEN requesting to get by a non-existent product ID, THEN the user receives a 404 status code')
    def test_get_product(self, auth):
        with step('Generating a non-existent ID'):
            non_exist_id = str(uuid.uuid4())

        with step(f'Getting product info by non-existent product ID via API "{non_exist_id}"'):
